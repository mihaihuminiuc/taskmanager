...
//	private static final String TAG = "DbHelper";
	...
	public static final SimpleDateFormat sdf = new SimpleDateFormat(TaskManager.DEFAULT_DATE_FORMAT);
	...
	private final String populateSQL = CommonUtils.concat("SELECT ",
	        ...
	        try {
				do {
					...
					if (rule == null && interval == null) {//one time
						...
						if (alarmMsg.getDateTime() < now-CommonUtils.MIN)
...
//		    	Log.e(TAG, e.getMessage(), e);
	...
	private final String listSQL = CommonUtils.concat("SELECT ",
													"a."+Alarm.COL_NAME+", ",
													"am."+AlarmMsg.COL_ID+", ",
	...
	public Cursor listNotifications(SQLiteDatabase db, String... args) {
		...
		String sql = CommonUtils.concat(listSQL,
								" WHERE "+selection,
	...
	public int cancelNotification(SQLiteDatabase db, String startTime, String endTime) {
	 ...
	 public static final String getDateStr(int year, int month, int date) {
		 return CommonUtils.concat(year, "-", month, "-", date);
	 }
	 
	 public static final String getTimeStr(int hour, int minute) {
		 return CommonUtils.concat(hour, ":", minute>9 ? "":"0", minute);
	 }	 
